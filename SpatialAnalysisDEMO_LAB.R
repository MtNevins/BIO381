...
library(sp)
coordinates(points)<- ~ Longitude + Latitude # ~ Long + Lat is column names 
points(x=points, pch=17, col="red")

### The following line usually works, maybe something is wrong with the website?
#bioclim <- getData('worldclim', var='bio', res=10) not working at this point
...
bioclimLC1<-crop(x=bioclimLC, extent(-74,-71, 42.5, 45.5)) # cropping it to Vermont's extent lat long min and max 
plot(bioclimLC1)
